...
class Kernel(object):
    __slots__ = ("ast", "integral_type", "oriented", "subdomain_id",
                 "domain_number", "tabulations", "points", "quadrature_rule",
...
class KernelBuilder(KernelBuilderBase):
    ...
    def __init__(self, integral_type, subdomain_id, domain_number, scalar_type=None):
        ...
        # KEY: swapped compared to Firedrake
        elif integral_type == 'interior_facet_horiz':
            self._entity_number = {'+': 0, '-': 1}
...
def prepare_coefficient(coefficient, name, scalar_type, interior_facet=False):
    ...
    if coefficient.ufl_element().family() == 'Real':
        ...
        # Here we use arrays rather than pointers to fit with how Themis calls kernels
        funarg = coffee.Decl(scalar_type, coffee.Symbol(name, rank=(1,)), qualifiers=["const"])
...
# THIS SHOULD PROBABLY BE CHANGED TO AN ARRAY WHEN THEMIS STARTS USING IT ie MANIFOLDS
